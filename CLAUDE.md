...
> ⚠️ **الريبو ده مالوش `index.html` من 25-09-2026** — الـ Pages URL القديم
> (`.../Bosta-Orders-Shipped-Scanner/`) بيدّي `404` بقرار، **بلا إعادة توجيه**.
...
```
tool  : bosta_tracker         → type: login · logout   (تاريخي بس — مفيش صفوف جديدة من 25-09-2026، بعد ما index.html اتشال. الدخول كله دلوقتي warehouse_ops_center من الهب)
...
> 🔴 **ممنوع `index.html` يرجع هنا.** لو الأداة احتاجت واجهة مستقلة يومًا،
> ده قرار جديد مش رجوع للقديم — والفرق إن الهب دلوقتي **هو** الواجهة الوحيدة
> الموجودة، مش نقطة رجوع بجانب واجهة تانية.
> ⚠️ **وصفر إعادة توجيه من الرابط القديم بقرار** — `https://ecommoda-dev.github.io/Bosta-Orders-Shipped-Scanner/`
> بيدّي **404** من دلوقتي (GitHub Pages مالهاش `index.html` يقرأه). أي بوكمارك
> قديم على أجهزة المخزن بيوصل لصفحة فاضية — نفس الثمن اللي اتقبل وقت شيل
> `shipped.html`/`returned.html` في v1.23.0/v1.25.0، بالحرف.

- 🔴 **`index.js` ما اتلمسش ولا سطر بسبب الشيل ده.** الـ Worker هو نفسه اللي
  كان بيخدم الصفحتين، وشيل `index.html` **مش تعديل Worker** — الـ endpoints
  والمنطق والـ CORS كلهم زي ما هم.
- 🔴 **`WORKER_SECRET` سره واحد دلوقتي — مفتاح `localStorage` واحد بس.**
  الهب بيبعته من `warehouse_ops_worker_secret` (سر مجموعة `warehouse_ops`)،
  و**مفتاح `bosta_shipped_scanner_worker_secret` القديم بقى بلا مستهلك** —
  مفيش صفحة تقراه تاني. ✅ **والمكسب المباشر:** البند اللي كان مفتوح («السر
  لازم يتلزق في مفتاحين طول ما الأداتين شغالين») اتقفل من نفسه.
- 🔴 **`MIN_WORKER_VERSION` هنا بقى بلا مستهلك حقيقي** — مفيش صفحة في الريبو
  ده بتقراه، والحد الأدنى الوحيد اللي الموظف بيشوفه هو `WOC_WORKERS.shipped.min`
  في الهب. القيمة فضلت في الكود كتوثيق داخلي للـ Worker نفسه، بس مقارنتها
  بأي حاجة هنا **مالهاش أثر على أي واجهة**.
- ⚠️ **فحص المتصفح بتاع الصفحة عايش في الهب من زمان** — `docs/scanners-check.mjs`
  (في `Warehouse-Operations-Center`). الريبو ده مالوش فحص متصفح خاص بيه من
  الأول، فالشيل ده **صفر أثر على الفحوصات**.
- ✅ **وخطر R1 (نسخة تانية من نفس المنطق تفترق في صمت) اتقفل بالإزالة** —
  مكانش فيه غير نسخة وحدة تتفرق منها دلوقتي.
...
- ⚠️ **الطابور مالوش مستهلك في الريبو ده** — الـ endpoint موجود في الـ Worker
  ومحدش بينادیه من هنا (الريبو ده Worker وبس، صفر واجهة). عشان كده
  `MIN_WORKER_VERSION` بلا أثر عملي هنا — راجع فوق في قسم الدمج.
...
آخر مطابقة: 25-09-2026 · `index.js` v3.6.2 · **`index.html` اتشال — الريبو ده
Worker وبس من دلوقتي**
...
ولا سطر. `CLAUDE.md` و`README.md` اتحدّثوا: الروابط بقت تشاور على صفحة الهب،
ومفتاح `localStorage` القديم (`bosta_shipped_scanner_worker_secret`) بقى بلا
مستهلك. راجع القسم فوق («الريبو ده Worker وبس من الآن فصاعدًا»).

24-09-2026 — v3.6.2 (`check-log-values.mjs` اتستبدل بالنسخة
